...
//字符串到长整形的互换
DWORD StringToDWORD(tstring str)
{
...
tstring GetCurrentPath()
{
	...
	::GetModuleFileName(NULL, szMyDllFileName, MAX_1024_LEN);
	str = szMyDllFileName;
...
bool LoadIniString(char *sApp, char *sKey, char *sReturn, int len, char *sFile, char *sDefault)
{
	...
	if (GetPrivateProfileString(sApp, sKey, sDefault1, strString, len, sFile))
	{
...
bool LoadIniInt(char *sApp, char *sKey, int &nReturn, char *sFile, char *sDefault)
{
	...
	if (GetPrivateProfileString(sApp, sKey, sDefault1, strString, 256, sFile))
	{
		if (strlen(strString) > 0)
		{
	...
	*(TCP_PACK_HEADER *)data = tcppackhead;
	...
	while (nLeft > 0)
	{
		...
		if (ret == SOCKET_ERROR)
		{
			...
			//			m_Log.writelog(true, LOG_DEBUG, "tcp thread send socket info error and thread exit!");
			goto RETURN;
		}
		idx += ret;
		nSend += ret;
