...
/*	_tcscpy(info.nickname, _T(""));		//网络名称					30
	_tcscpy(info.username, _T(""));		//真实名字 未用				30(60)
	info.sex = 0;							//性别						2(62)

	info.birthday = 0;			//出身年月					4(66)
	_tcscpy(info.memo, _T(""));			//简介						100(166)	

	info.companyid = 0;					//公司编号					4(170)
	info.departmentid = 0;						//部门编号					4(174)

	info.country = 0;						//国家名称					2(176)
	info.province = 0;						//省份名称					2(178)
	_tcscpy(info.city, _T(""));				//城市名称					30(208)
	_tcscpy(info.zipcode, _T(""));		//邮编号码					10(218)

	_tcscpy(info.mobile, _T(""));			//手机						20(228)
	_tcscpy(info.phone, _T(""));			//电话						20(248)
	_tcscpy(info.fax, _T(""));				//传真号码					20(268)

	_tcscpy(info.homepage, _T(""));			//主页						256(524)
	_tcscpy(info.email, _T(""));				//电子邮件					256(780)

	_tcscpy(info.dutyname, _T(""));			//职务名称		            30(880)
	_tcscpy(info.company, _T(""));		//公司名称					40(920)

	info.commflag = 0;					//常用权限(用位表示)		4(784)
	info.extflag = 0;					//常用类别和状态(用位表示)	4(792)
	info.onlineflag = 0;				//备用字段						4(796)
	info.langtype = LANGUAGE_NORMAL;				//个人信息的语言版本			2(798)
...
/*
	_tcscpy(info.nickname, pInfo->info.nickname);		//网络名称					30
	_tcscpy(info.username, pInfo->info.username);		//真实名字 未用				30(60)
	info.sex = pInfo->info.sex;							//性别						2(62)

	info.birthday = pInfo->info.birthday;			//出身年月					4(66)
	_tcscpy(info.memo, pInfo->info.memo);			//简介						100(166)	

	info.companyid = pInfo->info.companyid;					//公司编号					4(170)
	info.departmentid = pInfo->info.departmentid;						//部门编号					4(174)

	info.country = pInfo->info.country;						//国家名称					2(176)
	info.province = pInfo->info.province;						//省份名称					2(178)
	_tcscpy(info.city, pInfo->info.city);				//城市名称					30(208)
	_tcscpy(info.zipcode, pInfo->info.zipcode);		//邮编号码					10(218)

	_tcscpy(info.mobile, pInfo->info.mobile);			//手机						20(228)
	_tcscpy(info.phone, pInfo->info.phone);			//电话						20(248)
	_tcscpy(info.fax, pInfo->info.fax);				//传真号码					20(268)

	_tcscpy(info.homepage, pInfo->info.homepage);			//主页						256(524)
	_tcscpy(info.email, pInfo->info.email);				//电子邮件					256(780)

	_tcscpy(info.dutyname, pInfo->info.dutyname);			//职务名称		            30(880)
	_tcscpy(info.company, pInfo->info.company);		//公司名称					40(920)

	info.commflag = pInfo->info.commflag;					//常用权限(用位表示)		4(784)
	info.extflag = pInfo->info.extflag;					//常用类别和状态(用位表示)	4(792)

	info.onlineflag = pInfo->info.onlineflag;				//备用字段						4(796)
	info.langtype = pInfo->info.langtype;				//个人信息的语言版本			2(798)
...
{
	_tcscpy(info.company, _T(""));				//公司名称   40
	_tcscpy(info.leader, _T(""));				//法人		 未用(30)

	info.duty = 0;									//职务名称   2
	info.size = 0;									//公司规模   2
	info.trade = 0;									//所处行业   2

	_tcscpy(info.bankcode, _T(""));					//开户银行	40		   
	_tcscpy(info.bank_num, _T(""));				//银行账号	30
	_tcscpy(info.tax_num, _T(""));					//税务代号	30

	_tcscpy(info.mobile, _T(""));					//电话		未用
	_tcscpy(info.phone, _T(""));					//电话		20
	_tcscpy(info.fax, _T(""));						//传真		20
	_tcscpy(info.homepage, _T(""));					//主页名称  60
...
void CTradeInfo::CopyInfo(CTradeInfo *pInfo)
{
	...
	_tcscpy(info.company, pInfo->info.company);				//公司名称   40
	_tcscpy(info.leader, pInfo->info.leader);				//法人		 未用(30)

	info.duty = pInfo->info.duty;									//职务名称   2
	info.size = pInfo->info.size;									//公司规模   2
	info.trade = pInfo->info.trade;									//所处行业   2

	_tcscpy(info.bankcode, pInfo->info.bankcode);					//开户银行	40		   
	_tcscpy(info.bank_num, pInfo->info.bank_num);				//银行账号	30
	_tcscpy(info.tax_num, pInfo->info.tax_num);					//税务代号	30

	_tcscpy(info.mobile, pInfo->info.mobile);					//电话		未用
	_tcscpy(info.phone, pInfo->info.phone);					//电话		20
	_tcscpy(info.fax, pInfo->info.fax);						//传真		20
	_tcscpy(info.homepage, pInfo->info.homepage);					//主页名称  60
	...
	_tcscpy(info.memo, pInfo->info.memo);				//公司简介  500
}
...
void FRD_COMMON_INFO::ClearInfo()
{
	...
	_tcscpy(mobile, _T(""));			//20 城市（新版未用）
	_tcscpy(nickname, _T(""));		//30 姓名
...
//	_tcscpy(this->nickname, "");		//30 姓名
	_tcscpy(this->nickname, pInfo->nickname);		//30 姓名
...
void FRD_ONLINE_INFO::CopyInfo(FRD_ONLINE_INFO *pInfo)
{
	...
	_tcscpy(this->strcompany,pInfo->strcompany);						//1	备用
}
...
void CWebOAInfo::CopyInfo(FRD_ONLINE_INFO *pInfo)
{
	...
	this->onlinestatus = pInfo->onlinestatus;						//1
	_tcscpy(this->strstatus, pInfo->strstatus);						//1	备用
}
*/
...
///	msgformat = MSG_TXT;			//1 消息格式
	...
	sendtime = 0;					//4 发送时间
	isread = 0;						//2 是否被读过
	_tcscpy(strmsg, _T(""));		//910 消息内容
	_tcscpy(strmobile, _T(""));	//20 手机号码, 发短消息使用，用户接受信息时用于姓名
...
void GROUP_COMMON_INFO::ClearInfo()
{
	...
	type = GROUP_NULL;							//1 类型
	bak = 0;							//1 备用
	id = 0;								//4 
	_tcscpy(strname, _T(""));		//30 名称
