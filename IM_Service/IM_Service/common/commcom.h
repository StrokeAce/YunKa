// commit pack commlib.h: 数据传输中公用包的头文件
//
//////////////////////////////////////////////////////////////////////
...
public:
	...
	TCHAR			mobile[MAX_PHONE_LEN];		//20 城市（新版未用）
	TCHAR			nickname[MAX_USERNAME_LEN]; //20 姓名
...
public:
	...
	TCHAR			nickname[40];	//20 姓名(not use on new copy)
...
//	bool ConvertGBToBIG5(bool bGBToBIG5);
	void ClearInfo();
//	void CopyInfo(WEBOA_INFO *pInfo);
public:
...
public:
	...
	TCHAR			strmsg[MAX_MSG_RECVLEN+1];		//910 消息内容
	TCHAR			strmobile[MAX_MSGNICKNAME_LEN+1];	//66 手机号码, 发短消息使用，用户接受信息时用于姓名
	TCHAR			strfontinfo[MAX_MSGFONT_LEN+1]; //HTML,JSON=WX,CHSET=%u,H=%u,EF=%u,CR=%u,FN=%s
...
public:
	...
	void ClearInfo();
	void CopyInfo(GROUP_COMMON_INFO *pInfo);
...
public:
	...
	unsigned char	bak;							//1 备用
	unsigned int	id;								//4 
	TCHAR			strname[MAX_USERNAME_LEN];		//20 名称

	unsigned short	port;							//2	会议的端口
	//30
};

//多人讨论组、公告栏、会议的查询结构
//max size : 64
class GROUP_QUERY_INFO
{
public:
	GROUP_QUERY_INFO();
	~GROUP_QUERY_INFO();

	bool unpack(CPack &pack, unsigned short version);
	bool pack(CPack &pack, unsigned short version);


	void ClearInfo();
	void CopyInfo(GROUP_QUERY_INFO *pInfo);
...
public:
	...
	unsigned int	createuin;						//4 
	TCHAR			strusername[MAX_USERNAME_LEN];	//20 名称
	
	unsigned int	starttime;						//4
	unsigned char	onlinenum;						//2
	unsigned char	totalnum;						//2
...
public:
	...
	void ClearInfo();
	void CopyInfo(GROUP_INFO *pInfo);
...
public:
	...
	unsigned int	id;								//4 
	TCHAR			strname[MAX_USERNAME_LEN];		//20 名称
	TCHAR			strmemo[MAX_MEMO_LEN];			//255 名称

	unsigned int	createuin;						//4 
	TCHAR			strusername[MAX_USERNAME_LEN];	//20 名称

	unsigned int	createtime;						//4
	unsigned int	starttime;						//4
	unsigned char	onlinenum;						//1
	unsigned char	totalnum;						//1

	unsigned char	isrecord;						//1
	unsigned char	isvideo;						//1
	unsigned char	isnotice;						//1
	unsigned char	status;							//1

	unsigned short	port;							//2	会议的端口
	unsigned short	bak1;							//2

	//32+40+255 = 72+255 = 327
};
*/
//有关一个组、部门、用户的通用朋友信息
//max size : 32
...
public:
	...
	TCHAR			name[MAX_USERNAME_LEN+1]; //20 姓名
...
public:
	...
	unsigned int uin;
	TCHAR nickname[MAX_USERNAME_LEN];
...
public:
    ...
    TCHAR	strauth[MAX_RESPONSE_LEN];	//255 验证信息 
