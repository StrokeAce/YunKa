// comm pack commlib.cpp: 数据传输中使用的包的实现函数
//
//////////////////////////////////////////////////////////////////////
#include "../StdAfx.h"
#include <stdlib.h>
...
SRV_DENY_REG::SRV_DENY_REG(unsigned short ver)
{
	...
	_tcscpy(reason, _T(""));		//255  拒绝的类型描述

	_tcscpy(strAddress, _T(""));		//255 下载地址
...
SRV_CONF_LOGON::SRV_CONF_LOGON(unsigned short ver)
{
	...
	_tcscpy(stridmobile,_T(""));
	_tcscpy(stridmail,_T(""));
	_tcscpy(stridbak,_T(""));

	status  = STATUS_ONLINE;						//2  状态
	_tcscpy(strStatus,_T(""));

	_tcscpy(strMemo, _T(""));				//255 描述
	_tcscpy(strAddress,_T(""));				//255 下载地址
...
SRV_DENY_LOGON::SRV_DENY_LOGON(unsigned short ver)
{
	...
	uin = 0;					//4 用户UIN
	_tcscpy(strid, _T(""));	//60 字符类型的的登录

	deny = 0;					//2 拒绝登录的原因：0: 用户已经在线；1：UIN号码不存在； 2：密码错误  3: 服务器到最大  4:隐身登陆失败  5:其它类型 10 版本太旧
	_tcscpy(reason, _T(""));			//255
	_tcscpy(strAddress, _T(""));			//255下载地址
...
/*
bool SRV_LIST_FRD::AddInfo(USER_TABLE_PT pInfo)
{
	if(pInfo == NULL)
		return false;

	FRD_COMMON_INFO info;
	info.uin = pInfo->logininfo.uin;
	_tcscpy(info.mobile, "");
	_tcscpy(info.nickname, pInfo->userinfo.nickname);

	info.deptid = pInfo->userinfo.departmentid;
	info.externid = pInfo->userinfo.externid;

	info.groupid = 0;
	info.iconid = 0;

	info.sex = pInfo->userinfo.sex;
	info.langtype = pInfo->userinfo.langtype;
	info.bak = 0;
