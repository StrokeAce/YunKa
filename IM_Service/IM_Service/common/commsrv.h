...
*/
	...
	TCHAR	strMemo[MAX_RESPONSE_LEN];	//描述
};
...
public:
	...
	TCHAR			reason[MAX_RESPONSE_LEN];		//255  失败的类型描述
	TCHAR			strmemo[MAX_RESPONSE_LEN];		//255  失败的类型描述
};
...
public:
	...
	TCHAR strid[MAX_SID_LEN];			//60
	TCHAR pwd[MAX_PASSWORD_LEN];			//20 密码

	TCHAR strMemo[MAX_RESPONSE_LEN];		//255 描述
	TCHAR strAddress[MAX_URL_LEN];		//255 下载地址
...
public:
	...
	TCHAR			strid[MAX_SID_LEN];		//60  字符类型的的登录
	TCHAR			stridmobile[MAX_PHONE_LEN];	//20  字符类型的的登录
	TCHAR			stridmail[MAX_EMAIL_LEN];	//255 字符类型的的登录
	TCHAR			stridbak[MAX_SID_LEN];	//60  字符类型的的登录

	unsigned short status ;						//2  状态
	TCHAR strStatus[MAX_STATUS_LEN];				//40  状态描述
...
public:
	...
	TCHAR reason[MAX_RESPONSE_LEN];			//255

	TCHAR strAddress[MAX_URL_LEN];			//255下载地址
	//72+510 = 582
};
...
public:
	...
	TCHAR	strstatus[MAX_STATUS_LEN];			//在线状态的文字
...
public:
	...
	TCHAR strstatus[MAX_STATUS_LEN];	//120  //在线状态的文字描述,新的密码等
...
public:
	...
	TCHAR	mobile[MAX_PHONE_LEN];
	TCHAR	sms[MAX_SMS_LEN];
...
public:
	...
	TCHAR   szChatId[MAX_CHATID_LEN+1];  //会话id
	TCHAR   szRand[MAX_WEBCLIENID_LEN+1];//访客唯一标示
	TCHAR   szThirdid[MAX_THIRDID_LEN+1];//第三方id
...
struct  GROUP_LIST
{
	...
	TCHAR groupname[MAX_GROUPNAME_LEN+1]; //组名
};
