...
LOGIN_INFO* CSysConfigFile::GetLoginInfo(int type, unsigned long uid, tstring sid)
{
...
ALERT_INFO *CSysConfigFile::SetAllDefaultAlertInfo(int type)
{
	...
	tstring strPath = GetCurrentPath();
	strPath.TrimRight("\\");
	strPath += "\\sound";
...
void CSysConfigFile::AddServerAddress(tstring strServer, bool bMoveToTop)
{
	if (strServer.empty())
		return;
