...
#include "../chat_common/commsrv.h"
#include "../chat_common/commfloat.h"
#include "../chat_common/commform.h"
...
int CChatManager::RecvFloatShareList(PACK_HEADER packhead, char *pRecvBuff, int len)
{
	...
	for (iter = RecvInfo.sharememlist.begin(); iter != RecvInfo.sharememlist.end(); iter++)
	{
		...
		pUser = GetUserObject(ShareUserOb.uin);
		if (pUser == NULL)
		{
...
void CChatManager::SendTo_GetShareList()
{
...
void CChatManager::SendTo_GetListChatInfo()
{
...
{
	CUserObject *pUser = this->GetUserObject(id);
	...
	if (m_myInfoIsGet >= 0 && pUser->UserInfo.uid != 0 && pUser->UserInfo.uid == this->m_UserInfo.UserInfo.uid)
	{
		m_myInfoIsGet = -20;
	}
	return pUser;
...
CUserObject * CChatManager::GetUserObject(unsigned long id)
{
	if (id == 0)
	{
...
void CChatManager::TimerProc(void* p)
{
	int a = 10;
}

int CChatManager::GetAllUser()
{
	int nError = 0;
	...
	for (iter_user = m_mapUsers.begin(); iter_user != m_mapUsers.end(); iter_user++)
	{
		...
		{
			SendTo_GetUserInfo(pUser->UserInfo.uid);
		}
	}
	return nError;
}
